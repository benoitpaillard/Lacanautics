...
def contour_paths(mask,k):
    ...
    for arr in source_rings:
        ...
        eligible+=1; nsrc=len(pts); raw+=nsrc; it=smoothing_passes(k,nsrc); sm=chaikin_closed(pts,it)
        closed=np.vstack([sm,sm[0]]); simp=approximate_polygon(closed,tolerance=.035)
        if np.linalg.norm(simp[0]-simp[-1])<1e-8: simp=simp[:-1]
        if len(simp)<3: raise RuntimeError(f'threshold {k}: a source ring collapsed during vectorization')
...
def build_svg(pal,cls):
    ...
    for k in range(8):
        paths,raw,n,rs,source_rings=contour_paths(cls>=k,k); color='#%02x%02x%02x'%tuple(int(v) for v in pal[k])
...
def qc(svg,pal,cls,layers):
    ...
    for k in range(8):
        ...
        th.append({'threshold_m':k,'iou_1x_render':iou,'boundary_p95_px_1x_render':float(np.percentile(d,95)),'boundary_p99_px_1x_render':float(np.percentile(d,99)),'boundary_max_px_1x_render':float(d.max()),'source_rings':layers[k]['source_rings'],'emitted_rings':layers[k]['emitted_rings']})
    ...
    # Strict geometric raster round-trip QC only where we actually smooth contours.
    for q in th[:5]:
        if q['iou_1x_render']<.975 or q['boundary_p95_px_1x_render']>1.5: raise RuntimeError(f'smoothed threshold QC failed {q}')
    # Thresholds >=5 are emitted directly from exact source half-pixel rings, so topology
    # preservation is the meaningful invariant; 1x pixel-centre coverage is reported only.
    for q in th[5:]:
...
def patch_ui():
    ...
    for old in ['lacanautics-v4.3-subpixel','lacanautics-v4.3-bounded2','lacanautics-v4.3-bounded3','lacanautics-v4.3-depthaware','lacanautics-v4.3-depthaware2']:
        sw=sw.replace(old,'lacanautics-v4.3-final')
    (ROOT/'sw.js').write_text(sw)


def main():
    meta,pal,cls=load_classes(); svg,layers=build_svg(pal,cls); q=qc(svg,pal,cls,layers); OUT_SVG.write_text(svg)
    report={'version':'4.3-final','source_version':'4.1-fixed','method':'half-pixel contours; bounded Chaikin smoothing only for thresholds 0–4; thresholds 5–7 exact; source-ring topology asserted',
